#!/bin/bash
# BWP_trial.sh (BWP = Block-Wise Pruning Ratio Strategy)
# One trial: ratio pruning with given (down, up, mid), generate one image, then compute ImageReward.
set -euo pipefail
...
SEED=24432
PRUNE_METHOD="ratio"
# unet_pruning_experiment.py expects --experiments as "down up mid 0 0 0"
...
# Run pruning + generation + ImageReward
